...
def _csv_key(platform: str, folder: str, filename: str) -> str:
...
def _collect_key(platform: str, folder: str, filename: str) -> str:
...
def download_object(bucket: str, key: str, local_path: str):
    """객체 로컬 다운로드"""
    _client().download_file(Bucket=bucket, Key=key, Filename=local_path)


def upload_object(bucket: str, key: str, local_path: str, content_type: str):
    """로컬 파일 업로드, Content-Type 지정"""
    _client().upload_file(Filename=local_path, Bucket=bucket, Key=key, ExtraArgs={"ContentType": content_type})
...
def copy_object(src_bucket: str, key: str, dst_bucket: str, dst_key: str | None = None):
    """버킷 간 복사, 대상 키 지정 가능"""
    _client().copy_object(Bucket=dst_bucket, Key=dst_key or key, CopySource={"Bucket": src_bucket, "Key": key})
...
def presigned_get_url(bucket: str, key: str, expires: int) -> str:
    """GET 서명 URL"""
    return _client().generate_presigned_url("get_object", Params={"Bucket": bucket, "Key": key}, ExpiresIn=expires)
