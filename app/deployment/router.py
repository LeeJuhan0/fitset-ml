...
@router.post("/api/v1/{platform}/deploy", response_model=ApiResponse[DeployData])
def deploy(
    body: DeployRequest,
...
@router.get("/api/v1/{platform}/model/latest", response_model=ApiResponse[ModelLatestData])
def model_latest(
...
@router.get("/api/v1/{platform}/model/version-stats", response_model=ApiResponse[VersionStatsData])
def version_stats(
    platform: str = Depends(validate_platform),
    trace_id: str = Depends(get_trace_id),
):
