...
async def start_training(
    ...
    """학습 시작, 202"""
    return ApiResponse(trace_id=trace_id, data=await service.start_training(session, platform, payload))


@router.get("/{platform}/train/status", response_model=ApiResponse[TrainStatusData])
def train_status(
    query: Annotated[JobIdQuery, Query()],
    platform: str = Depends(validate_platform),
    trace_id: str = Depends(get_trace_id),
):
    """MLflow run 상태 조회"""
    return ApiResponse(trace_id=trace_id, data=service.get_status(platform, query))
...
def run_metric_history(
    run_id: str,
    query: Annotated[MetricQuery, Query()],
    platform: str = Depends(validate_platform),
    trace_id: str = Depends(get_trace_id),
):
    """run 메트릭 시계열"""
    return ApiResponse(trace_id=trace_id, data=service.metric_history(run_id, query))
